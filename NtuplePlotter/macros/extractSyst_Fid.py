...
'Nsignal':'Statistical likelihood fit',
...
def getTable(args):
    ...
    isVisResult = False
    for line in _file:
        if 'Fiducial Cross Section Ratio Value' in line:
...
#place syst up in second spot and syst down in first spot
    ...
    if takeMaxSyst:
        ...
        for w in sorted(unc.items(),key=lambda x: x[1][2],reverse=True):
            sign1 = '+'
            sign2 = '+'
            if max(w[1][:2])<0:sign1=' '
            if min(w[1][:2])<0:sign2=' '
            if w[0]=='Nsignal': 
                sign2 = '-'
                totalUp += max(w[1][:2])**2
                totalDown += max(w[1][:2])**2
            else:
                if max(w[1][:2])>0:
                    totalUp += max(w[1][:2])**2
                if min(w[1][:2])<0:
                    totalDown += min(w[1][:2])**2
            if twikiFormat:
                table +=  "| %s  |  %.1f  |  %.1f  |   \n" % (SystNames[w[0]], w[1][0], w[1][1])
            else:
                table +=  "%s   & %c%.1f / %c%.1f  \\\\ \n" % (SystNames[w[0]], sign1, max(w[1][:2]), sign2, min(w[1][:2]))
        if not twikiFormat:
            table += '\\hline\n'
            table += 'Total & +%.1f / -%.1f \\\\ \n'% (totalUp**0.5,totalDown**0.5)
            table +=  '\\hline \n'
            table +=  '\\end{tabular} \n'
