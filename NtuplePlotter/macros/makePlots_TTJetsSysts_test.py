...
import templateFits
import qcd_fit
import calc_the_answer_fiducial_quick as calc_the_answer_fiducial
import vgamma_fit
...
if SaveOutput:
	...
	if 'Up' in saveSystName:
		sys.stdout = open(folder+'ratio_'+saveSystName.replace('Up','_up')+'.txt','w')
	elif 'Down' in saveSystName:
		sys.stdout = open(folder+'ratio_'+saveSystName.replace('Down','_down')+'.txt','w')
	else:sys.stdout = open(folder+'ratio_'+saveSystName+'.txt','w')
	# if isSyst:
...
ttjetsNum = {'ScaleUp'   : 39136663.,
	     'ScaleDown'     : 41838271.,
	     'MatchingUp': 20336562.,
	     'MatchingDown'  : 37063003.,
	     'TopMassUp' : 24399341.,
	     'TopMassDown'   : 26369020.,
	     }
...
def saveNoMETTemplates(inputDir, inputData, outFileName, histName,syst = ''):
	varList = ['MET','M3',lep+'1RelIso','ele1MVA']
	DataTempl = loadDataTemplate(varList, inputData, histName)
	MCTemplDict = loadMCTemplates(varList, inputDir, histName,'',1001,syst)
	MCTempl = []
...
def saveBarrelFitTemplates(inputDir, inputData,  outFileName):
	...
	MCTempl_b = loadMCTemplates(varList, inputDir, 'hist_1pho_barrel_top_','',1001,systematic)	#change
	MCTempl_rs_b = loadMCTemplates(varList, inputDir, 'hist_1pho_rs_barrel_top_', '_signal', 1001,systematic) #change
	MCTempl_fe_b = loadMCTemplates(varList, inputDir, 'hist_1pho_fe_barrel_top_', '_electron', 3005, systematic)#change
	MCTempl_fjrb_b = loadMCTemplates(varList, inputDir, 'hist_1pho_fjrb_barrel_top_', '_fake', 3005, systematic)#change
...
def savePreselTemplates(inputDir, qcdDir, inputData, outFileName):
	...
	MCTemplDict = loadMCTemplates(varList, inputDir, 'hist_1pho_top_','',1001,systematic) #change
	MCTempl = []
	MCTempl.append(MCTemplDict['WHIZARD'])
	MCTempl.append(MCTemplDict['TTJets'])
	MCTempl.append(MCTemplDict['Vgamma'])
...
def saveBarrelTemplates(inputDir, qcdDir, inputData, outFileName,syst):
	if WJetsSF != 1.0 or TopSF != 1.0:
		print 'We want to save templates for M3 fit, but the SFs are not 1.0'
		print 'exiting'
		return
	...
	print 'HERE2'
	MCTemplDict_b = loadMCTemplates(varList+['MCcategoryfid'], inputDir, 'hist_1pho_'+region+'_top_','',1001,syst)#change
	print 'HERE3'
	MCTempl_b = []
...
#	MCTempl_b.append(MCTemplDict_b['Vgamma'])
	...
	MCTempl_rs_b = loadMCTemplates(varList+['MCcategoryfid'], inputDir, 'hist_1pho_rs_barrel_top_', '_signal', 1001,syst)
	MCTempl_fe_b = loadMCTemplates(varList+['MCcategoryfid'], inputDir, 'hist_1pho_fe_barrel_top_', '_electron', 3005,syst)
	MCTempl_fjrb_b = loadMCTemplates(varList+['MCcategoryfid'], inputDir, 'hist_1pho_fjrb_barrel_top_', '_fake', 3005,syst)
...
def makeAllPlots(varList, inputDir, qcdDir, dataDir, outDirName):
	...
	MCTemplDict = loadMCTemplates(varList+['MCcategoryfid'], inputDir, 'hist_1pho_top_','',1001,systematic) #NoMET change
	MCTempl = []
	MCTempl.append(MCTemplDict['WHIZARD'])
	MCTempl.append(MCTemplDict['TTJets'])
#	MCTempl.append(MCTemplDict['Vgamma'])
...
#	MCTempl.remove(MCTemplDict['Vgamma'])	
	...
	print 'HERE2'
	MCTemplDict_b = loadMCTemplates(shortVarList+['MCcategoryfid'], inputDir, 'hist_1pho_'+region+'_top_','',1001,systematic)#change
	print 'HERE3'
	MCTempl_b = []
	MCTempl_b.append(MCTemplDict_b['WHIZARD'])
	MCTempl_b.append(MCTemplDict_b['TTJets'])
#	MCTempl_b.append(MCTemplDict_b['Vgamma'])
	...
	MCTempl_rs_b = loadMCTemplates(shortVarList+['MCcategoryfid'], inputDir, 'hist_1pho_rs_barrel_top_', '_signal', 1001,systematic)
	MCTempl_fe_b = loadMCTemplates(shortVarList+['MCcategoryfid'], inputDir, 'hist_1pho_fe_barrel_top_', '_electron', 3005,systematic)
	MCTempl_fjrb_b = loadMCTemplates(shortVarList+['MCcategoryfid'], inputDir, 'hist_1pho_fjrb_barrel_top_', '_fake', 3005,systematic)
 	print "SF after photon selection :", "Top=", TopSF ,"WJets=",WJetsSF, "QCD=",QCDSF, "OtherMC=",otherMCSF	
	# save final templates, exactly as they are on the plots and by categories
	saveTemplatesToFile([DataTempl_b] + MCTempl_b + MCTempl_rs_b.values() + MCTempl_fe_b.values() + MCTempl_fjrb_b.values(), 
...
def makePhotonSelectionPlots(varList, inputDir, qcdDir, dataDir, outDirName):
	...
	if QCDSF > 0.0001:
		QCDTempl_b = loadQCDTemplate(shortVarList, qcdDir, 'hist_1pho_'+region+'_top_') #change
	MCTemplDict_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_'+region+'_top_','',1001,systematic)#change
	MCTempl_b = []
	MCTempl_b.append(MCTemplDict_b['WHIZARD'])
	MCTempl_b.append(MCTemplDict_b['TTJets'])
#	MCTempl_b.append(MCTemplDict_b['Vgamma'])
	...
	MCTempl_rs_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_rs_barrel_top_', '_signal', 1001,systematic)
	MCTempl_fe_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_fe_barrel_top_', '_electron', 3005,systematic)
	MCTempl_fjrb_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_fjrb_barrel_top_', '_fake', 3005,systematic)
 	print "SF after photon selection :", "Top=", TopSF ,"WJets=",WJetsSF, "QCD=",QCDSF, "OtherMC=",otherMCSF	
	# save final templates, exactly as they are on the plots and by categories
	saveTemplatesToFile([DataTempl_b] + MCTempl_b + MCTempl_rs_b.values() + MCTempl_fe_b.values() + MCTempl_fjrb_b.values(), 
		['MET','M3','photon1Et','photon1Eta','WtransMass','MCcategory','nJets','ele1pho1Mass','photon1ChHadRandIso','photon1ChHadSCRIso','photon1SigmaIEtaIEta'], 
...
saveAccTemplates(InputHist, 'ttbar_acceptance.root',systematic)
...
if WJetsSF == 1.0 and TopSF == 1.0:
	#saveNoMETTemplates(InputHist, DataHist, 'templates_presel_nomet.root', 'hist_1phoNoMET_top_',systematic)
...
if isSyst:
	if isElectron:
		QCDSF = 1.55678740691
	if isMuon:
		QCDSF = 0.780698354839#0.683983420073
...
saveBarrelTemplates(InputHist, QCDHist, DataHist, 'templates_barrel_unscaled.root',systematic)
...
TopSFErr = (TTGSFErr**2 + TopSFerror_m3**2)**0.5 
...
calc_the_answer_fiducial.channel = lep

calc_the_answer_fiducial.TTJets1l_num = TTJets1l_num
calc_the_answer_fiducial.TTJets2l_num = TTJets2l_num
calc_the_answer_fiducial.TTJetsHad_num = TTJetsHad_num

calc_the_answer_fiducial.photnPurity = phoPurity
calc_the_answer_fiducial.photnPurityErr = phoPurityError
calc_the_answer_fiducial.eleFakeSF = 1.458
calc_the_answer_fiducial.eleFakeSFErr = 0.1985
if systematic == 'EleFakeSF_up':
	calc_the_answer_fiducial.eleFakeSF = 1.458+0.1985
if systematic == 'EleFakeSF_down':
	calc_the_answer_fiducial.eleFakeSF = 1.458-0.1985

calc_the_answer_fiducial.M3TopSF = TopSF_presel
calc_the_answer_fiducial.M3TopSFErr = TopSF_presel_error
calc_the_answer_fiducial.M3WJetsSF = WJetsSF_presel
calc_the_answer_fiducial.M3WJetsSFErr = WJetsSF_presel_error
calc_the_answer_fiducial.M3_photon_topFrac = m3_topFrac
calc_the_answer_fiducial.M3_photon_topFracErr = m3_topFracErr

calc_the_answer_fiducial.ttjetsSyst = systematic
calc_the_answer_fiducial.ttjetsNum = ttjetsNum

calc_the_answer_fiducial.barrelFileName_M3fitscaled = 'templates_barrel_scaled_afterPhotonM3.root'
calc_the_answer_fiducial.barrelFileName_M3fitscaled = 'templates_barrel_scaled.root'

calc_the_answer_fiducial.ttgammaSeq = seq(0.65, 1., 0.01)

progressBar = not SaveOutput

xsRatio, xsRatioErr, bestttgSF, bestttgSFErr, bestvgammaSF, bestvgammaSFErr, bestjgSF, bestjgSFErr = calc_the_answer_fiducial.doTheCalculation(progressBar = progressBar)
...
mcEventsTable.inputFileName = calc_the_answer_fiducial.barrelFileName_M3fitscaled
