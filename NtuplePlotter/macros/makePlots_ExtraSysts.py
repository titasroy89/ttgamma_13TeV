...
import templateFits
import qcd_fit
#import calc_the_answer_fiducial as calc_the_answer_fiducial_quick
...
if SaveOutput:
	...
	####These are swapped, because I accidentally crossed the wires on making the ttjets histograms
	if 'Up' in saveSystName:
		sys.stdout = open(folder+'ratio_'+saveSystName.replace('Up','_down')+'.txt','w')
	elif 'Down' in saveSystName:
		sys.stdout = open(folder+'ratio_'+saveSystName.replace('Down','_up')+'.txt','w')
...
def saveNoMETTemplates(inputDir, inputData, outFileName, histName,syst = 'special'):
...
def makeQCDPlots(varList,qcdDir,outDir):
	...
	MCTemplDict = loadMCTemplates(varList, qcdDir, 'hist_1phoNoMET_top_','',1001,'special') #NoMET change
        MCTempl = []
        MCTempl.append(MCTemplDict['WHIZARD'])
        MCTempl.append(MCTemplDict['TTJets'])
#        MCTempl.append(MCTemplDict['Vgamma'])
...
saveTTgammaAccTemplates(InputHist, 'ttgamma_acceptance.root',systematic)
...
if isMuon:
	...
	# phoPurity	    =  0.529181796587
	# phoPurityError	    =  0.0553638825451
...
calc_the_answer_fiducial_quick.ttjetsSyst = systematic
calc_the_answer_fiducial_quick.ttjetsNum = ttjetsNum

if isElectron:
	calc_the_answer_fiducial_quick.fidSelectionEff    = 0.120033372906
	calc_the_answer_fiducial_quick.fidSelectionEffErr = 0.00188903435387
if isMuon:
	calc_the_answer_fiducial_quick.fidSelectionEff    = 0.126818970789  
	calc_the_answer_fiducial_quick.fidSelectionEffErr = 0.00187962261225


#calc_the_answer_fiducial_quick.ttgammaSeq =  seq(1.4,4,.02)
...
# outCSVtable = open('datafile.csv','w')
# table += '%f,%f,,\n'%(m3_topFrac,m3_topFracErr)
# table += '%f,%f,,\n'%(phoPurity,phoPurityError)
# outCSVtable.write(table)
# outCSVtable.close()
